"""
GeneReg - Gene Regulation Predictor v2.1
Local ChIP-seq peak data + Motif + Literature weighted scoring
v2.1: Memory-efficient - scan bed files on demand instead of loading entire index
"""

import os, re, json, math, requests, gzip
...
# ==================== ChIP-seq (Local Peak Data - Memory Efficient) ====================
class ChIPSeqAnalyzer:
    def __init__(self):
        self.tf_dirs = {}  # lowercase name -> actual dir name
        self._scan_tf_dirs()

    def _scan_tf_dirs(self):
        """Build TF directory mapping (lightweight, just scans dir names)"""
    ...
    def _search_local_peaks(self, gene_info: Dict, tf_dir_name: str, window: int = 100000) -> List[Dict]:
        """Scan bed files for a specific TF to find peaks near the gene TSS"""
        ...
        peaks_found = []
        chr_with_prefix = f"chr{chr_name}"
                    ...
                    for line in f:
                        ...
                        if dist <= window:
                            peaks_found.append({
                                ...
                                "tf_name": tf_dir_name,
                                "signal": float(parts[6]) if len(parts) > 6 else 0
                            })
            except Exception:
                pass
    ...
    def analyze(self, gene: str, receptor: str, gene_info: Optional[Dict] = None) -> Dict:
        ...
        # 1. Primary: Search local peak data for THIS receptor only
        if gene_info and gene_info.get("chr") and gene_info.get("tss") and tf_dir_name:
            for window in [5000, 20000, 50000, 100000]:
                found = self._search_local_peaks(gene_info, tf_dir_name, window)
                if found:
                    peaks = found
                    break
            ...
            if peaks:
                ...
                if min_dist < 1000: dist_score = 35
                elif min_dist < 2000: dist_score = 32
                elif min_dist < 5000: dist_score = 28
                elif min_dist < 10000: dist_score = 22
                elif min_dist < 50000: dist_score = 15
                else: dist_score = 8
                
                # Peak count
                count_score = min(35, len(peaks) * 7)
                ...
                # TF has data but no binding near this gene
        ...
        # 2. Fallback: ENCODE API for experiment info
...
# ==================== FastAPI ====================
app = FastAPI(title="GeneReg - Gene Regulation Predictor v2.1")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

gf = GeneInfoFetcher()
ca = ChIPSeqAnalyzer()  # Memory-efficient: no pickle index loaded
ma = MotifAnalyzer()
...
@app.get("/api/health")
def health(): return {"status":"ok","service":"GeneReg","version":"2.1","tf_count":len(ca.tf_dirs)}
...
<nav><div class="nav-container"><a href="https://43.99.62.219/" class="back-btn"><i class="fa fa-arrow-left"></i> Back to Animal Hormone DB</a></div></nav>
...
<footer class="border-t border-gray-200 py-4 mt-12 bg-white"><div class="max-w-6xl mx-auto px-6 flex items-center justify-between text-xs text-gray-400"><span>GeneReg v2.1 · Local ENCODE Peaks + JASPAR + PubMed + DeepSeek AI</span><span>Inner Mongolia University of Science and Technology</span></div></footer>
<script>
let lastData=null;
...
const bars=[{name:'ChIP-seq Binding',score:s.chip_seq.score,color:'#8b5cf6',detail:`${s.chip_seq.peaks_found} peaks, ${s.chip_seq.experiments_found} ENCODE experiments`+(s.chip_seq.min_distance_to_tss!=null?`, nearest ${s.chip_seq.min_distance_to_tss}bp`:'')+(s.chip_seq.local_data_available?' (local)':'')},{name:'Motif Prediction',score:s.motif.score,color:'#3b82f6',detail:s.motif.motif_found?`JASPAR ${s.motif.motif_info.matrix_id}${s.motif.promoter_matches.length?', '+s.motif.promoter_matches.length+' promoter matches':''}`:'No motif found'},{name:'Literature Evidence',score:s.literature.score,color:'#f59e0b',detail:`${s.literature.articles_found} PubMed articles`}];
...
const cp=s.chip_seq;document.getElementById('chipContent').innerHTML=`<p class="text-sm text-gray-600 mb-2">Source: ${cp.source} | Peaks found: <strong>${cp.peaks_found}</strong>${cp.experiments_found?` | Experiments: <strong>${cp.experiments_found}</strong>`:''}${cp.min_distance_to_tss!=null?' | Nearest peak: <strong>'+cp.min_distance_to_tss+'bp</strong> from TSS':''}</p>${cp.experiments&&cp.experiments.length?`<p class="text-xs font-medium text-gray-700 mb-1">ENCODE ChIP-seq Experiments for ${d.receptor}:</p><div class="space-y-1 mb-3">${cp.experiments.map(e=>`<div class="text-xs bg-gray-50 rounded px-3 py-1.5"><a href="https://www.encodeproject.com/experiments/${e.accession}/" target="_blank" class="text-violet-600 hover:text-violet-800 font-medium">${e.accession}</a> <span class="text-gray-500">- ${e.biosample}</span></div>`).join('')}</div>`:''}${cp.peaks.length?`<p class="text-xs font-medium text-gray-700 mb-1">Binding Peaks near ${d.gene}:</p><div class="overflow-x-auto"><table class="w-full text-xs"><thead><tr class="border-b border-gray-200"><th class="text-left py-1 px-2">Chr</th><th class="text-left py-1 px-2">Start</th><th class="text-left py-1 px-2">End</th><th class="text-left py-1 px-2">Distance</th><th class="text-left py-1 px-2">TF</th></tr></thead><tbody>${cp.peaks.map(p=>`<tr class="border-b border-gray-100 hover:bg-gray-50"><td class="py-1 px-2">${p.chr}</td><td class="py-1 px-2">${p.start.toLocaleString()}</td><td class="py-1 px-2">${p.end.toLocaleString()}</td><td class="py-1 px-2 font-medium ${p.distance_to_tss<5000?'text-emerald-600':p.distance_to_tss<50000?'text-amber-600':'text-gray-500'}">${p.distance_to_tss? p.distance_to_tss.toLocaleString()+'bp':''}</td><td class="py-1 px-2 text-violet-600">${p.tf_name||''}</td></tr>`).join('')}</tbody></table></div>`:'<p class="text-sm text-gray-400">No binding peaks found near this gene from this receptor.</p>'}`;
