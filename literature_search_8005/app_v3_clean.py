"""
文献检索服务 - Literature Search Service
基于PubMed + AI的基因文献检索平台
部署于43服务器 8005端口
"""
...
{{"genes":[{{"symbol":"GENE1","full_name":"Full Name","role":"role description","finding":"what was found"}}],"key_finding":"main finding"}}"""
    ...
    def extract_genes_from_literature(self, query: str, articles: List[Dict]) -> Dict:
        """并发分析所有文献 - 每篇文章独立调用AI"""
        all_genes = {}
        all_findings = []
        ...
        for sym in all_genes:
            all_genes[sym]["evidence"] = all_genes[sym]["evidence"][:3]
...
app = FastAPI(title="文献检索服务 - Literature Search Service")
...
@app.post("/api/search")
def search_literature(request: SearchRequest):
    """文献检索主接口 - 先AI优化检索词再搜索"""
    ...
    search_term = optimized.get("search_term", request.query)
    
    # 2. 用优化后的检索词搜PubMed
    articles = searcher.search(search_term, request.max_results)
    ...
    if not articles:
        return {
            ...
            "articles": [],
            "gene_analysis": None,
            "message": "未找到相关文献，请尝试不同的关键词"
        }
    
    # 3. AI分析提取基因
    gene_analysis = None
    if request.include_ai_analysis and articles:
        gene_analysis = analyzer.extract_genes_from_literature(request.query, articles)
...
def search_literature_get(
    ...
    max_results: int = Query(15, description="最大结果数"),
    include_ai: bool = Query(True, description="是否包含AI分析")
):
    """文献检索GET接口"""
    if not query.strip():
        raise HTTPException(status_code=400, detail="查询不能为空")
    ...
    search_term = optimized.get("search_term", query)
    
    articles = searcher.search(search_term, max_results)
    ...
    gene_analysis = None
    if include_ai and articles:
        gene_analysis = analyzer.extract_genes_from_literature(query, articles)
...
def deep_search(request: DeepSearchRequest):
    """深度检索：AI生成多个检索词，分别搜索后合并去重"""
    if not request.query.strip():
        raise HTTPException(status_code=400, detail="查询不能为空")
    
    # 1. AI生成多个检索词
    queries = deep_search_queries(request.query)
    if not queries:
        # fallback到普通搜索
        queries = [{"label": "Default", "search_term": request.query}]
    ...
    for q in queries:
        ...
        articles = searcher.search(term, request.max_results_per_query)
        new_count = 0
        for a in articles:
    ...
    # 3. AI分析全部合并后的文献
    gene_analysis = analyzer.extract_genes_from_literature(request.query, all_articles)
    
    # 精简articles，去掉abstract和mesh_terms减小响应体积
    slim_articles = [{"pmid": a["pmid"], "title": a["title"], "authors": a["authors"], "journal": a["journal"], "year": a["year"], "doi": a["doi"], "url": a["url"], "abstract": a["abstract"][:150]+"..." if a.get("abstract") and len(a["abstract"])>150 else a.get("abstract","")} for a in all_articles]
...
@app.post("/api/genes")
def search_genes(request: GeneSearchRequest):
    """基因数据库搜索"""
    if not request.query.strip():
        raise HTTPException(status_code=400, detail="查询不能为空")
    genes = searcher.search_genes(request.query, request.max_results)
    return {"query": request.query, "total_genes": len(genes), "genes": genes}
...
def health():
    return {
        ...
        "version": "2.0",
        "endpoints": {
            "search": "POST/GET /api/search (AI优化检索词)",
            "deep_search": "POST /api/deep-search (多角度深度检索)",
            "genes": "POST /api/genes",
